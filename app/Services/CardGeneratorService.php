...
class CardGeneratorService
{
    ...
    {
        $existingCount = Card::where('bingo_id', $bingo->id)->count();
        $generated = 0;
        $attempts = 0;
        ...
        $existingHashes = Card::where('bingo_id', $bingo->id)
            ->with('numbers')
            ->get()
            ->map(function ($card) {
        ...
        while ($generated < $quantity && $attempts < $maxAttempts) {
            ...
            $cardNumber = str_pad($existingCount + $generated + 1, 3, '0', STR_PAD_LEFT);
